    ...
    public class AlbumRequester
    {
        ...
        public void GetAllAlbums()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                {
                    Console.WriteLine("{0} {1} {2} {3}", a.Id, a.Title, a.Producer, a.Year);
                }
            }
            else
            {
        ...
        public void GetAlbumById(int id)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                Console.WriteLine("{0} {1} {2} {3}", album.Id, album.Title, album.Producer, album.Year);
            }
            else
            {
                Console.WriteLine("{0} ({1})", (int)response.StatusCode, response.ReasonPhrase);
        ...
        public void AddSongToAlbum(int albumId, int songId)
        {
            ...
            {
                var album = response.Content.ReadAsAsync<Album>().Result;
                Console.WriteLine("{0} {1}", album.Id, album.Songs);
            }
            else
            {
                Console.WriteLine("{0} ({1})", (int)response.StatusCode, response.ReasonPhrase);
