...
namespace InfiniteConvergentSeries
{
    ...
    class InfiniteConvergentSeries
    {
        private static void Main()
        {
            ...
            //  Specific lambda functions depending on the terms of each series are used as delegates in the ConvergentSum() method: 

            Console.WriteLine("\n1 + 1/2 + 1/4 + 1/8 + 1/16 + ... = {0}\n", ConvergentSum(i => 1.0 / Math.Pow(2, i), precision));
            Console.WriteLine("1 + 1/2! + 1/3! + 1/4! + 1/5! + ... = {0}\n", ConvergentSum(i => 1.0 / Factorial(i + 1), precision));
            Console.WriteLine("1 + 1/2 - 1/4 + 1/8 - 1/16 + ... = {0}\n", ConvergentSum(i => i == 0 ? 1 : -1.0 / Math.Pow(-2, i), precision));
        }
